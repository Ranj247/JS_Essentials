...
/*
The first step to understanding how to work with forms in JavaScript is to learn how to get the values that a user has submitted. 
*/

/*---------------------Take a simple login form for example:-------------*/
...
/*
Most form fields can be accessed in this way, but keep in mind that for some fields like checkboxes and multi-selection dropdowns, you may need to use more complex logic to access their values or whether or not they are "selected". 

You'll learn more about these other form operations as you write more complex code, and there are also JavaScript libraries that can serialize an entire form into a nice JavaScript object in a single line of code for these more complex situations! 

The basic example above is demonstrated in the runnable example below, where we've added an event listener to listen to the submit event on the form, and when it's triggered, execute a JavaScript function called handleSubmit(). The important thing to understand now is that a form element in your HTML is no different than any other element. It has attributes, properties and methods, all of which are accessible and mutable via JavaScript:
*/
